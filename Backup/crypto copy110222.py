...
def treemap_vol():
    ...
    treemap_vol.update_layout(
        title = 'Hieraquical Average Volume in the last 24 hours',
        template='none',
...
def bar_returns():
    ...
    bar_returns.update_layout(
        margin=dict(l=60, r=80, t=40, b=250),
        title = ' Cumulative Average Return in the last 24 hours (%)',
        height = 550,
        width = 850,
        font_family="Verdana", # Set Font style
        font_size=18, # Set Font size) # legend false 
...
def market_actual():
    ...
    fig.update_layout(margin=dict(l=0, r=0, t=20, b=10),height=150)
...
def render_content(tab):
    ...
    # Create Dcc Dropdownmenu      
            ...
            html.Div(children=[
                ...
                html.Div(
                    dcc.Graph(
                        figure=bar_returns()), style={'display': 'inline-block','margin-left': '80px'}),  
                ...
                html.Div(
                    dcc.Graph(
                        figure=treemap_vol()), style={'display': 'inline-block','margin-left': '0px'}),
...
def volatility(selected_crypto):
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Returns Trend in the last 24 hours', # Subplot main title
...
def forecast(selected_crypto, p, q):
    ...
    fig.update_layout(title={
            'text': 'Residuals Diagnostics', # Subplot main title
            'y':0.98, # Set main title y-axis position
            'x':0.5, # Set main title x-axis position
            'xanchor': 'center', # xachor position
            'yanchor': 'top'}, # yachor position 
