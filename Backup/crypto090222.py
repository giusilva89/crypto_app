...
def get_data():
    ...
    data10 = yf.download(tickers=['BNB-USD'], 
                          period = '24h',   
                           interval = '15m',
                           group_by = 'ticker')
...
def indicators():
    ...
    data10 = df[df['Crypto'] == 'BNB-USD'].set_index('Datetime')
    ...
    data8 = data8['Close'].resample('1d').mean().dropna(how='all')
    data9 = data9['Close'].resample('1d').mean().dropna(how='all')
    data10 = data10['Close'].resample('1d').mean().dropna(how='all')
    data11 = data11['Close'].resample('1d').mean().dropna(how='all')
    data12 = data12['Close'].resample('1d').mean().dropna(how='all')
    data13 = data13['Close'].resample('1d').mean().dropna(how='all')
    ...
    fig_09.append_trace(fig_09_trace_12, row = 1, col=3)
    fig_09.append_trace(fig_09_trace_04, row = 1, col=4)
    fig_09.append_trace(fig_09_trace_13, row = 1, col=5)
    ...
    fig_09.update_layout(margin=dict(l=0, r=0, t=0, b=10),
                         height=200)
...
def treemap():
    treemap = px.treemap(df, path=['Crypto'], 
                     values='Close', color='Crypto',
                     color_continuous_scale='Viridis')
    treemap.update_layout(margin=dict(l=0, r=0, t=0, b=20),
                          height = 410,
                          width = 1100,
    ...
    # Create Dcc Dropdownmenu      
            ...
            html.Div(children=[
                html.Div(
                    ...
                    style_table={ 'font-family': 'verdana', 'width': '650px'}), 
                    style={'display': 'inline-block',
                           'margin-left': '1px'}),
                html.Div(
                    dcc.Graph(
                        figure=treemap()), style={'display': 'inline-block',
                                                  'margin-left': '400 px'})], style={'display': 'inline-block'})])
...
def volatility(selected_crypto):
    ...
    volatility_trend = px.line(returns, 
                               x = returns.Datetime,
                               y = returns.Close, 
                               markers=True, 
                               color_discrete_sequence=["#090059"])
    ...
    volatility_trend.update_xaxes(showspikes=True)
    volatility_trend.update_yaxes(showspikes=True)
