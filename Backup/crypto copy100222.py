...
def indicators():
    ...
    fig_09.update_layout(margin=dict(l=0, r=0, t=20, b=10),
                         height=200)
...
def treemap_close():
    df_tree = df[['Crypto', 'Close']].groupby('Crypto', as_index=False).mean()\
    .sort_values('Close', ascending=False).round(2)
    #df_tree = df_tree.query("Crypto!='BTC-USD'")
    treemap_close = px.treemap(df_tree, path=['Crypto'], 
                     values='Close', color='Crypto',
                     color_continuous_scale='Viridis')
    treemap_close.update_layout(
...
def bar_returns():
    ...
    bar_returns.update_layout(
        margin=dict(l=60, r=80, t=60, b=200),
        title = 'Cumulative Return (%) in the last 24 hours',
        height = 600,
        width = 870,
        font_family="Verdana", # Set Font style
        font_size=18, # Set Font size) # legend false 
...
def bar_volume():
    ...
    bar_volume.update_layout(
        margin=dict(l=60, r=80, t=60, b=200),
        title = 'Average Volume ($) in the last 24 hours',
        height = 600,
        width = 870,
        font_family="Verdana", # Set Font style
        font_size=18, # Set Font size) # legend false 
...
def render_content(tab):
    ...
    # Create Dcc Dropdownmenu      
            ...
            html.Div(children=[
                ...
                html.Div(
                    dcc.Graph(
                        figure=treemap_close()), style={'display': 'inline-block','margin-left': '100px'}),
                ...
                html.Div(
                    dcc.Graph(
                        figure=bar_returns()), style={'display': 'inline-block','margin-left': '0px'}),                  
