...
def get_data():
    ...
    # Reset index and rename it to 'Datetime' for each DataFrame
    dataframes = [data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13]
...
def plotSeasonalDecompose(
    ...
    result = seasonal_decompose(
            x, model=model, filt=filt, period=100,
...
def autocorrelation_plots(series):
    ...
    fig.update_annotations(font=dict(family="Arial}", size=24))
...
app.layout = html.Div(children=[
    html.H2(children='Cryptocurrency Time Series Analysis', style={'font-family': "Arial", 
                                                                   'text-align': 'center',
                                                                   'color': '#090059'}),
    
    dbc.Row( html.Marquee("Yahoo Finance Live Coin Watch "), style={'font-family': "Arial", 
                                                  'text-align': 'center', 
                                                  'color': '#090059'}),
...
def render_content(tab):
                ...
                dcc.Dropdown(
                    id='crypto-dropdown',
                     ...
                     style=dict(
                         ...
                         fontFamliy = 'Arial'),
                     value='BTC-USD')], style={'width': '100%',
                                               'font-family': 'arial'})
            , html.Div(className='six columns', children=[
        dcc.Dropdown(id='crypto-dropdown-2',
                     options=[{'label': i, 'value': i}
                              for i in df['Crypto'].unique()],
                     placeholder="Select  Cryptocurrency",
                     style=dict(
            ...
            html.Div(dcc.Graph(id='price-graph'),style={'display': 'inline-block','margin-left': '25px'}),
            html.Div(dcc.Graph(id='price-graph-2'),style={'display': 'inline-block','margin-left': '25px'}),
    ...
    html.Div(id="Dropdown_menu"),],
            ...
            # Volatility Graph
            html.Div(dcc.Graph(id='volatility-graph'), style={'display':'inline-block'}),
            
            html.Div(dcc.Graph(id='decomposition-graph'), style={'display':'inline-block'}),
...
def update_graph(selected_crypto):
    ...
    roll_5 = filtered_df.Close.rolling(window=5, min_periods=1).mean()
    roll_20 = filtered_df.Close.rolling(window=20, min_periods=1).mean()
    ...
    # MACD 
    macd = MACD(close=filtered_df['Close'], 
                window_slow=26,
                window_fast=12, 
                window_sign=9)
    # Stochastic
    stoch = StochasticOscillator(high=filtered_df['High'],
...
@app.callback(
    ...
    Input(component_id='crypto-dropdown-2', component_property='value')
)



def update_graph_2(selected_crypto):
...
def volatility(selected_crypto):
    ...
    fig.add_trace(go.Scatter(x=returns.Datetime, y=returns.Close,
                        mode='lines',
                        line=dict(color='#157DEC', width=1),
                        name='Returns'))
...
@app.callback(
    Output(component_id='decomposition-graph', component_property='figure'),
    Input(component_id='crypto-dropdown', component_property='value'),
)
def seasonal_2(selected_crypto):

    filtered_df_2 = df[df['Crypto'] == selected_crypto]
    returns = pd.DataFrame(100 * filtered_df_2.Close.pct_change().dropna()).reset_index()

    return plotSeasonalDecompose(returns.Close)
...
def forecast(selected_crypto, mean, p, o, q, vol, power, dist):
    ...
    fig.update_annotations(font=dict(family="Arial}", size=1))
